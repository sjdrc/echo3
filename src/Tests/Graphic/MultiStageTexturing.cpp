...
namespace MultiStageTexturingTest
{
	class MultiStageTexturingTestApplication : public Application
	{
	public:
		MultiStageTexturingTestApplication(){}
		...
		bool Initialise()
		{
			...
			if (!Application::Initialise("MultiStageTexturingTestApplication", "data/MultiStageTexturingTestApplication.config"))
			{
				return false;
			}
	...
	TEST_CASE("MultiStageTexturing")
	{
		// Turn off log output, we will just use output from this test.
		gDefaultLogger.SetLogMask(Echo::Logger::LogLevels::NONE);

		MultiStageTexturingTestApplication multiStageTexturingTestApplication;
		REQUIRE(multiStageTexturingTestApplication.Initialise());
		multiStageTexturingTestApplication.Execute();
	}
}
